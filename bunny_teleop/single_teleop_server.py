...
class TeleopServer:
    def __init__(self, port: int, host="localhost"):
        ...
        self._shared_last_init_config: Optional[SingleInitializationConfig] = None
    ...
    def handle_init_config_request(self):
        try:
            while True:
                ...
                try:
                    init_config = SingleInitializationConfig.from_dict(init_config_dict)
                except TypeError as e:
                    raise ValueError(
                        f"Teleop ZMQ Server: Invalid initialization config. "
                        f"It must be an instance of SingleInitializationConfig.\n"
                        f"{e}"
                    )
